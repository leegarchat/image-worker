...
# Целевые платформы (статический musl)
TARGET_X64="x86_64-unknown-linux-musl"
TARGET_X86="i686-unknown-linux-musl"
TARGET_ARM64="aarch64-unknown-linux-musl"
TARGET_ARM32="armv7-unknown-linux-musleabihf"

TARGETS=("$TARGET_X64" "$TARGET_X86" "$TARGET_ARM64" "$TARGET_ARM32")

# Определение движка сборки: cross (если есть) или cargo
if command -v cross &>/dev/null; then
    BUILDER="cross"
    echo "==> Используется 'cross' (контейнерная кросс-компиляция)"
else
    BUILDER="cargo"
    echo "==> 'cross' не найден, используется нативный 'cargo'"
    echo "==> Проверка системных toolchain-компонентов..."
...
build_target() {
    ...
    if [ "$BUILDER" = "cross" ]; then
        ...
        # Конфигурация линкеров при локальной сборке через cargo
        case "$target" in
            ...
            "x86_64-unknown-linux-musl")
                # Для хостовой x64-системы линкер обычно встроен или используется musl-gcc
    ...
    if [ -f "$src_bin" ]; then
        ...
        # Стриппинг отладочных символов для уменьшения размера файла
        if command -v strip &>/dev/null; then
            strip "$dst_bin" 2>/dev/null || true
        fi

        echo "Успешно: $dst_bin ($(stat -c%s "$dst_bin" 2>/dev/null || stat -f%z "$dst_bin") байт)"
    else
        echo "Ошибка: бинарник не найден по пути $src_bin"
        exit 1
    fi
}

# Сборка под 4 архитектуры
...
echo ""
echo "============================================================"
echo "Все бинарники собраны и перемещены в каталог dist/:"
ls -lh "$DIST_DIR"
echo "============================================================"
        case "$target" in
            "$TARGET_AARCH64_UNKNOWN_LINUX_MUSL"|"$TARGET_ARM64")
                export CARGO_TARGET_AARCH64_UNKNOWN_LINUX_MUSL_LINKER="aarch64-linux-gnu-gcc"
                ;;
            "$TARGET_ARM32")
                export CARGO_TARGET_ARMV7_UNKNOWN_LINUX_MUSLEABIHF_LINKER="arm-linux-gnueabihf-gcc"
